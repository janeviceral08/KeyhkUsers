...
export default class SearchRentalsHotel extends Component {
    constructor(props) {
        super(props);
...
firestore().collection('users').where('userId', '==', userId).onSnapshot(
    ...
    const self = this;
    var productQuery =  firestore().collection('products').where('rentalType', '==', 'Hotels').where('admin_control', '==', true).where('status', '==', true).where('city', '==', this.state.City.trim());
