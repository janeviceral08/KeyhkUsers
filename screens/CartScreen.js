...
updateTextInput = (text, field) => {
  ...
  this.setState(state);
    ...
    this.state.cart.forEach((item) => {
      if(item.sale_price){
        if(item.addons == null || item.addons.length < 1){
...
async onRemoveFromCart (id) {
  ...
  render() {
    ...
    console.log('fromPlace CartScreen: ', this.props.route.params.fromPlace);
    return (
      <Root>
      <Container>
               ...
               this.state.cart.map((item,i)=>{
                 return(
                   ...
                   <View style={{width:width,marginRight: 0,backgroundColor:'white', flexDirection:'row', borderBottomWidth:2, borderColor:"#cccccc",paddingHorizontal: 10}}>
                     ...
                     <View style={{flex:1, backgroundColor:'transparent', padding:10, justifyContent:"space-between"}} >   
                      <TouchableOpacity style={{alignSelf: 'flex-end', position: 'absolute'}} onPress={()=> this.openModal(item)}>
                             <Text style={{color:'#019fe8'}}><AntDesign name="plus" size={13} color="#019fe8"/>Add Note</Text>
                    </TouchableOpacity>                    
                       <View>                       
                         ...
                         {item.sale_price > 0 ?
                           [item.unit && item.unit == null?
                            <Text>
                              <Text style={{textDecorationLine: 'line-through', color:'gray', fontSize: 10}}>
                            ...
                            <Text>
                            <Text style={{textDecorationLine: 'line-through', color:'gray', fontSize: 10}}>
                            {this.props.route.params.currency}{item.price}/ {item.unit}    
                            </Text> 
                             {this.props.route.params.currency}{item.sale_price}/ {item.unit}
                          </Text> ]
                            :
                          [item.unit && item.unit == null?
                          <Text>
                 ...
                 {  console.log('Choice: ', item.choice)}
                       ...
                       <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                         ...
                         {item.sale_price ? [item.addons == null  || item.addons.length < 1? <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>{this.props.route.params.currency}{(item.sale_price*item.qty).toFixed(2)}</Text> :<Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>{this.props.route.params.currency}{((item.sale_price*item.qty)+(item.total_addons*item.qty)).toFixed(2)}</Text>] : [item.addons == null || item.addons.length < 1 ? <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>{this.props.route.params.currency}{(item.price*item.qty).toFixed(2)}</Text> : <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>{this.props.route.params.currency}{((item.price*item.qty)+(item.total_addons*item.qty)).toFixed(2)}</Text>]}
                         <View style={{flexDirection:'row', alignItems:'center'}}>
                          ...
                          <TouchableOpacity  onPress={()=>this.onConfirmation(item.id)}>
                             <MaterialCommunityIcons name="delete-circle-outline" size={30} color={"#019fe8"} />
                           </TouchableOpacity>
                       </View>
                     </View>
                   </View> :
                   ...
                   <View style={{width:width,marginRight: 0,backgroundColor:'white', flexDirection:'row', borderBottomWidth:2, borderColor:"#cccccc", paddingVertical:10,paddingHorizontal: 10}}>
                     ...
                     <View style={{flex:1, backgroundColor:'transparent', padding:10, justifyContent:"space-between"}} >                    
                       <View>                       
                         <Text style={{fontWeight:"bold", fontSize:15}}>{item.name}</Text>
                         {item.sale_price ?
                          <Text><Text style={{textDecorationLine: 'line-through'}}>{this.props.route.params.currency}{item.price}</Text> {this.props.route.params.currency}{item.sale_price}/ {item.unit}</Text> : <Text>{this.props.route.params.currency}{item.price} / {item.unit}</Text>}
                         
                         <Text>Note :{item.note}</Text>
                       </View>
                       <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                         {item.sale_price ? <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>{this.props.route.params.currency}{(item.sale_price*item.qty).toFixed(2)}</Text> : <Text style={{fontWeight:'bold',color:"#33c37d",fontSize:15}}>{this.props.route.params.currency}{(item.price*item.qty).toFixed(2)}</Text>}
                         <View style={{flexDirection:'row', alignItems:'center'}}>
            ...
            <View style={{flexDirection: 'row', flexGrow: 1, flexShrink: 1, justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10}}>
              <View style={{flexDirection: 'row', paddingRight: 20, alignItems: 'center', paddingLeft: 20}}>
                <Text style={{color: '#8f8f8f', paddingRight: 30, fontSize: 20}}>SubTotal: </Text>
                    ...
                    <NumberFormat  renderText={text => <Text style={{ paddingLeft: (width / 2)- 60, fontWeight: 'bold'}}>{text}</Text>} value={Math.round(this.calculateTotalPrice()*10)/10} displayType={'text'} thousandSeparator={true} prefix={this.props.route.params.currency} />
              </View>
            </View>
          </View>
          <View style={{ height: 40, alignItems: 'center'}}>
          {cart.length != 0 ?
            <TouchableOpacity style={[styles.centerElement, {backgroundColor: '#019fe8', width: width - 10, height: 40, borderRadius: 5, padding: 10}]} onPress={()=> this.checkAvailability()}>
